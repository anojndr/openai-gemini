...
const fixCors = ({ headers, status, statusText }) => {
  ...
  // Remove compression-related headers so clients don't attempt to decompress
...
async function handleCompletions(req, apiKey) {
  ...
  if (response.ok) {
    ...
    if (req.stream) {
      ...
      // Set "text/event-stream" to avoid compression issues
      const corsData = fixCors(response);
      corsData.headers.set("Content-Type", "text/event-stream; charset=utf-8");
      return new Response(body, corsData);
    } else {
...
const transformMsg = async ({ role, content }) => {
  ...
  if (content.every((item) => item.type === "image_url")) {
    // Must have at least some text part
    parts.push({ text: "" });
  }
  return { role, parts };
};
...
const generateChatcmplId = () => {
  ...
  return (
    "chatcmpl-" + Array.from({ length: 29 }, randomChar).join("")
  );
};
